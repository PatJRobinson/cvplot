...
auto Series::setValueSpecifyXAxis(const std::vector<double> &values, std::vector<double> &xaxis) -> Series & {
  if (xaxis.size() != values.size())  {
    std::cerr << "Axis and data values must have the same length, truncating" << std::endl;
    xaxis.resize(values.size());
  }
  std::vector<std::pair<double, double>> data(values.size());
  auto i = 0;
  for (auto &d : data) {
...
auto Figure::drawFitSpecifyYaxis(void *buffer, double ymin, double ymax) const -> int {
  auto x_min = (include_zero_x_ ? 0. : FLT_MAX);
  auto x_max = (include_zero_x_ ? 0. : FLT_MIN);
  auto y_min = (include_zero_y_ ? 0. : FLT_MAX);
  auto y_max = (include_zero_y_ ? 0. : FLT_MIN);
  ...
  if (n_max != 0) {
    draw(buffer, x_min, x_max, ymin, ymax, n_max, p_max);
  }
